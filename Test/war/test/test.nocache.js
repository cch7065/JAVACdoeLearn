...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "test" because\n';
 ...
 * "/test.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['test'].superdevmode = true;

  var expectedSuffix = '/test.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + test + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:test';
  var devModeSessionKey = '__gwtDevModeSession:test';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/test/test.nocache.js';
  ...
  $wnd.__gwt_activeModules['test'].canRedirect = true;
  $wnd.__gwt_activeModules['test'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/test?user.agent=' + ua + '&_callback=' + callback;
